...
var LocalStorage = class {
  ...
  get(state2) {
    ...
    const key = STORAGE_KEY_PREFIX + state2;
...
var CookieStorage = class {
  ...
  get(state2) {
    ...
    const value = this.#getCookie(STORAGE_KEY_PREFIX + state2);
    this.#setCookie(STORAGE_KEY_PREFIX + state2, "", this.#cookieExpiration(-100));
    if (value) {
      return JSON.parse(value);
    }
    return null;
   ...
   */
  add(state2) {
    this.#setCookie(STORAGE_KEY_PREFIX + state2.state, JSON.stringify(state2), this.#cookieExpiration(60));
  }
  /**
   * @param {string} key
   * @returns
...
// node_modules/@josempgon/vue-keycloak/dist/index.mjs
...
var setKeycloak = (value) => {
  keycloak.value = value;
};
var setToken = (token, tokenParsed) => {
  state.token = token;
...
var useKeycloak = () => {
  return {
    ...
    hasResourceRoles: (roles, resource) => isArray(roles) && !isNil(resource) && state.isAuthenticated && !isNil(state.resourceRoles) && isArray(state.resourceRoles[resource]) && roles.every((role) => state.resourceRoles[resource].includes(role))
  };
};
...
var vueKeycloak = {
  install: async (app, options) => {
    ...
    const keycloakInitOptions = !isNil(keycloakPluginConfig.initOptions) ? { ...defaultInitConfig, ...keycloakPluginConfig.initOptions } : defaultInitConfig;
...
//# sourceMappingURL=@josempgon_vue-keycloak.js.map
