...
import SystemNodeComponent from '../components/SystemNode.vue';
import type { SystemNode, Connection, UseCase, LogEntry } from '../types/system';
...
const nodes = ref<SystemNode[]>([
  ...
  { id: 'light_act', label: 'Light Analysis', type: 'activity', x: 20, y: 75 },
  { id: 'deep_act', label: 'Deep Analysis', type: 'activity', x: 45, y: 75 },
  { id: 'embeddings', label: 'Embeddings', type: 'activity', x: 65, y: 75 },
  { id: 'vectorstore', label: 'VectorStore', type: 'activity', x: 85, y: 75 },
...
const connections = ref<Connection[]>([
  ...
  { id: 'deep_act_to_gemini', from: 'deep_act', to: 'gemini' },
  { id: 'embeddings_to_nomic', from: 'embeddings', to: 'nomic' },
  { id: 'deep_act_to_docling', from: 'deep_act', to: 'docling' },
]);
...
const useCases = ref<UseCase[]>([
  ...
  },
  {
    ...
    activeConnections: ['web_to_gw', 'gw_to_doc_contr', 'doc_contr_to_doc_svc', 'doc_svc_to_light_act', 'doc_svc_to_deep_act',
      ...
      'deep_act_to_gemini', 'embeddings_to_nomic', 'deep_act_to_docling']
  },
  {
    id: 'author_search',
...
<template>
  <div class="bounding-panel">
    <div class="tailwind-page">
      <div class="w-full flex bg-slate-950 text-slate-100 font-sans overflow-hidden">
        ...
        <div class="w-80 border-r border-slate-900 bg-slate-900 p-6 flex flex-col gap-6 shrink-0">
          ...
          <div class="space-y-2">
            <button v-for="uc in useCases" :key="uc.id" @click="activeUseCase = uc; telemetryLogs = []" :class="[
              'w-full text-left p-2 rounded-xl text-sm transition-all border font-medium',
          ...
          <div class="p-4 rounded-xl bg-slate-800/40 border border-slate-800 mt-auto">
            <h3 class="text-xs font-bold uppercase tracking-wider text-blue-400 mb-2">Flow Scope</h3>
        ...
        <div class="flex-1 flex flex-col p-6 gap-6 h-full overflow-hidden">
          ...
          <div class="h-[600px] w-full relative border border-slate-900 rounded-2xl bg-slate-950 overflow-hidden">

            <svg class="absolute inset-0 w-full h-full pointer-events-none z-0">
              <g v-for="conn in connections" :key="conn.id">
                <line v-if="getNodeById(conn.from) && getNodeById(conn.to)" :x1="`${getNodeById(conn.from)!.x}%`"
                  :y1="`${getNodeById(conn.from)!.y}%`" :x2="`${getNodeById(conn.to)!.x}%`"
                  :y2="`${getNodeById(conn.to)!.y}%`" :class="[
                    'transition-all duration-500',
                    isConnectionActive(conn.id) ? 'stroke-blue-500 stroke-[3px]' : 'stroke-slate-800 stroke-[1.5px]'
                  ]" />
                <!-- Moving Vector Overlay Pulses -->
                <line v-if="getNodeById(conn.from) && getNodeById(conn.to) && isConnectionActive(conn.id)"
                  :x1="`${getNodeById(conn.from)!.x}%`" :y1="`${getNodeById(conn.from)!.y}%`"
                  :x2="`${getNodeById(conn.to)!.x}%`" :y2="`${getNodeById(conn.to)!.y}%`" stroke="#3b82f6"
                  stroke-width="3" stroke-dasharray="6 12" class="animate-dash" />
              </g>
            </svg>

            <!-- Dynamic Absolute Coordinate System Element Render Wrapper -->
            <SystemNodeComponent v-for="node in nodes" :key="node.id" :id="node.id" :label="node.label"
              :type="node.type" :isActive="isNodeActive(node.id)"
