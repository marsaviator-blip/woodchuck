...
var SplitComponent = class _SplitComponent {
  ...
  constructor() {
    ...
    this.gutterMouseDownSubject.pipe(filter((context) => !this.customGutter() || this.customGutter()._canStartDragging(context.mouseDownEvent.target, context.gutterIndex + 1)), switchMap((mouseDownContext) => (
      ...
      fromMouseMoveEvent(this.document).pipe(startWith(mouseDownContext.mouseDownEvent), pairwise(), skipWhile(([, currMoveEvent]) => gutterEventsEqualWithDelta(mouseDownContext.mouseDownEvent, currMoveEvent, this.gutterClickDeltaPx(), mouseDownContext.gutterElement)), take(1), takeUntil(merge(fromMouseUpEvent(this.document, true), fromEvent(this.document, "blur"))), tap(() => {
        this.ngZone.run(() => {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      contentQueries: function SplitComponent_ContentQueries(rf, ctx, dirIndex) {
        if (rf & 1) {
          ɵɵcontentQuerySignal(dirIndex, ctx._areas, SPLIT_AREA_CONTRACT, 4)(dirIndex, ctx.customGutter, SplitGutterDirective, 5);
        }
        if (rf & 2) {
          ɵɵqueryAdvance(2);
        }
...
var SplitAreaComponent = class _SplitAreaComponent {
  constructor() {
    ...
    this._internalSize = linkedSignal(() => {
      if (!this.visible()) {
        return 0;
      }
