      ...
      return [r].flat().reduce((s3, a2) => {
        if (a2 != null) {
          ...
          if (u === "string" || u === "number") s3.push(a2);
            ...
            s3 = p.length ? s3.concat(p.filter((f2) => !!f2)) : s3;
          }
        }
        return s3;
      }, i2);
  ...
  function r(n, l2) {
    ...
    for (; c3 <= n.length; ) {
      ...
      if ((g3 === '"' || g3 === "'" || g3 === "`") && n[c3 - 1] !== "\\" && (d2 = d2 === g3 ? null : g3), !d2 && (g3 === "(" && u++, g3 === ")" && u--, (g3 === "," || c3 === n.length) && u === 0)) {
        let f2 = m2.trim();
        f2.startsWith("dt(") ? o.push(le(f2, l2)) : o.push(s3(f2)), m2 = "", c3++;
        continue;
      }
      g3 !== void 0 && (m2 += g3), c3++;
    }
...
function de(e, t = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u in m2) {
      let g3 = m2[u], f2 = ve(g3), p = G(u, a2) ? Q(d2) : Q(d2, re(u));
      if (i(f2)) o.push({ node: f2, path: p });
      else {
        let y2 = ae2(p), R = Y2(f2, p, s3, [a2]);
        Re(l2, y2, R);
        let T2 = p;
...
var b = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  var R, T2, j, O, M2, z3, V2;
  let { preset: a2, options: n } = t, l2, o, c3, m2, d2, u, g3;
  if (s(a2) && n.transform !== "strict") {
    let { primitive: L, semantic: te, extend: re2 } = a2, f2 = te || {}, { colorScheme: K2 } = f2, A2 = v2(f2, ["colorScheme"]), x2 = re2 || {}, { colorScheme: X } = x2, G2 = v2(x2, ["colorScheme"]), p = K2 || {}, { dark: U } = p, B = v2(p, ["dark"]), y2 = X || {}, { dark: I } = y2, H2 = v2(y2, ["dark"]), W = s(L) ? this._toVariables({ primitive: L }, n) : {}, q2 = s(A2) ? this._toVariables({ semantic: A2 }, n) : {}, Z = s(B) ? this._toVariables({ light: B }, n) : {}, pe = s(U) ? this._toVariables({ dark: U }, n) : {}, fe = s(G2) ? this._toVariables({ semantic: G2 }, n) : {}, ye = s(H2) ? this._toVariables({ light: H2 }, n) : {}, Se = s(I) ? this._toVariables({ dark: I }, n) : {}, [Me, ze] = [(R = W.declarations) != null ? R : "", W.tokens], [Ke, Xe] = [(T2 = q2.declarations) != null ? T2 : "", q2.tokens || []], [Ge, Ue] = [(j = Z.declarations) != null ? j : "", Z.tokens || []], [Be, Ie] = [(O = pe.declarations) != null ? O : "", pe.tokens || []], [He, We] = [(M2 = fe.declarations) != null ? M2 : "", fe.tokens || []], [qe, Ze] = [(z3 = ye.declarations) != null ? z3 : "", ye.tokens || []], [Fe, Je] = [(V2 = Se.declarations) != null ? V2 : "", Se.tokens || []];
  ...
  if (s(t) && r.transform !== "strict") {
    let y2 = e.replace("-directive", ""), m2 = t, { colorScheme: R, extend: T2, css: j } = m2, O = v2(m2, ["colorScheme", "extend", "css"]), d2 = T2 || {}, { colorScheme: M2 } = d2, z3 = v2(d2, ["colorScheme"]), u = R || {}, { dark: V2 } = u, L = v2(u, ["dark"]), g3 = M2 || {}, { dark: te } = g3, re2 = v2(g3, ["dark"]), K2 = s(O) ? this._toVariables({ [y2]: h(h({}, O), z3) }, r) : {}, A2 = s(L) ? this._toVariables({ [y2]: h(h({}, L), re2) }, r) : {}, X = s(V2) ? this._toVariables({ [y2]: h(h({}, V2), te) }, r) : {}, [G2, U] = [(f2 = K2.declarations) != null ? f2 : "", K2.tokens || []], [B, I] = [(x2 = A2.declarations) != null ? x2 : "", A2.tokens || []], [H2, W] = [(p = X.declarations) != null ? p : "", X.tokens || []], q2 = this.transformCSS(y2, `${G2}${B}`, "light", "variable", r, i2, a2, n), Z = this.transformCSS(y2, H2, "dark", "variable", r, i2, a2, n);
  ...
  if (s(t)) {
    ...
    if (s3 !== "style") {
      ...
      t = r === "dark" ? c3.reduce((m2, { type: d2, selector: u }) => (s(u) && (m2 += u.includes("[CSS]") ? u.replace("[CSS]", t) : this.getSelectorRule(u, l2, d2, t)), m2), "") : C3(l2 != null ? l2 : ":root,:host", t);
    }
    if (o) {
...
//# sourceMappingURL=chunk-K3KVKTBG.js.map
